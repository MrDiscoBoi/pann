...
function AnnouncementCard(props: Prop) {
  ...
  const fetchUserResultList = async () => {
    const result = await Repo.announcement.getUserResult(announcement.id)
    if (result) {
      setUserResultList([])
  ...
  const onUpdate = async (ann: Partial<Announcement>) => {
    const result = await Repo.announcement.update(ann)
    if (result) {
  ...
  const onDelete = async () => {
    await Repo.announcement.delete(announcement.id)
  ...
  const handleSubmitImport = async () => {
    await Repo.announcement.upsertUserResult(announcement.id, userResultList)
    fetchUserResultList()
  }
